...
export function SidebarNav({
  ...
  const statusCounts = {
    critical: tickets.filter((t) => normalizePriority(t.priority) === "critica").length,
    pending: tickets.filter((t) => normalizeStatus(t.status) === "suspenso").length,
    scheduled: tickets.filter((t) => normalizeStatus(t.status) === "agendado").length,
  }
      ...
      <nav className="flex h-full flex-col p-4">
        <div className="space-y-1">
          {navItems.map((item) => (
            <Button
              ...
              className={cn("w-full justify-start gap-3", activeTab === item.id && "bg-sidebar-accent")}
        ...
        <div className="mt-6">
          ...
          >
            ...
            <ChevronDown className={cn("h-4 w-4 transition-transform", environmentsOpen && "rotate-180")} />
          </Button>

          {environmentsOpen && (
            <div className="mt-2 space-y-1 pl-2">
              <Button
                variant={selectedEnvironment === null ? "secondary" : "ghost"}
                size="sm"
              ...
              {environments.map((env) => {
                const count = tickets.filter((t) => t.environment === env).length
                return (
                  <Button
                    key={env}
                    variant={selectedEnvironment === env ? "secondary" : "ghost"}
                    size="sm"
                    className="w-full justify-between"
        ...
        <div className="mt-auto space-y-2 border-t border-sidebar-border pt-4">
          <div className="text-xs font-medium uppercase text-muted-foreground px-3 mb-2">Status Rápido</div>
          <div className="flex items-center justify-between rounded-md bg-destructive/10 px-3 py-2">
            ...
            <Badge variant="destructive">{statusCounts.critical}</Badge>
          </div>
          <div className="flex items-center justify-between rounded-md bg-warning/10 px-3 py-2">
            ...
            <Badge className="bg-warning text-warning-foreground">{statusCounts.pending}</Badge>
          </div>
          <div className="flex items-center justify-between rounded-md bg-success/10 px-3 py-2">
            ...
            <Badge className="bg-success text-success-foreground">{statusCounts.scheduled}</Badge>
          </div>
        </div>
      </nav>
    </aside>
