...
export function KPICards({ selectedEnvironment, tickets }: KPICardsProps) {
    ...
    const openedCurrent = filteredTickets.filter((ticket) =>
        inRange(toDate(ticket.suspendedAt), currentStart, now),
    );
    ...
    const scheduledCurrent = filteredTickets.filter(
        (ticket) =>
            ...
            inRange(toDate(ticket.scheduledDate), currentStart, now),
    );
    const scheduledPrevious = filteredTickets.filter(
        (ticket) =>
            normalizeStatus(ticket.status) === "agendado" &&
            inRange(toDate(ticket.scheduledDate), previousStart, currentStart),
    );

    const criticalCurrent = openedCurrent.filter(
        (ticket) => normalizePriority(ticket.priority) === "critica",
    );
    const criticalPrevious = openedPrevious.filter(
        (ticket) => normalizePriority(ticket.priority) === "critica",
    );
    ...
    const avgDaysCurrent = Math.round(
        openedCurrent.reduce(
            (accumulator, ticket) => accumulator + ticket.daysOpen,
            0,
        ) / (openedCurrent.length || 1),
    );
    const avgDaysPrevious = Math.round(
        openedPrevious.reduce(
            (accumulator, ticket) => accumulator + ticket.daysOpen,
            0,
        ) / (openedPrevious.length || 1),
    );
