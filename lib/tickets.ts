export type TicketStatus = "Suspenso" | "Agendado" | "Em Atendimento" | "Concluido" | string
...
export const getStatsByEnvironment = (tickets: SuspendedTicket[]) => {
  ...
  return environments.map((env) => ({
    ...
    critical: tickets.filter((t) => t.environment === env && normalizePriority(t.priority) === "critica").length,
    scheduled: tickets.filter((t) => t.environment === env && normalizeStatus(t.status) === "agendado").length,
  }))
}
...
export const getTimelineData = (tickets: SuspendedTicket[]) => {
  ...
  return days.map(({ date, label }) => {
    ...
    const suspensos = tickets.filter((t) => sameDay(new Date(t.suspendedAt))).length
    const agendados = tickets.filter(
      (t) => t.scheduledDate && sameDay(new Date(t.scheduledDate)) && normalizeStatus(t.status) === "agendado",
    ).length
    const concluidos = tickets.filter(
      (t) => sameDay(new Date(t.suspendedAt)) && normalizeStatus(t.status) === "concluido",
    ).length
