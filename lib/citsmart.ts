...
const mergeCookies = (base: Map<string, string>, incoming: Map<string, string>) => {
...
const buildAuthHeaders = (cookieHeader: string, authToken?: string | null, accept = "application/json") => {
  const headers = new Headers()
  headers.set("Cookie", cookieHeader)
...
export async function createCitSmartSession(): Promise<CitSmartLoginSession> {
  const baseUrl = getEnvOrDefault("CITSMART_BASE_URL") ?? "https://cco.4biz.globalweb.com.br"
  ...
  if (!userName || !password) {
    throw new Error("Configure CITSMART_USERNAME e CITSMART_PASSWORD no ambiente do servidor.")
  }
  ...
  const preflightCookies = extractCookies(getSetCookieHeaders(preflightResponse), ["JSESSIONID"])
  const preflightCookieHeader = preflightCookies.size > 0 ? buildCookieHeader(preflightCookies) : undefined
  ...
  if (!loginResponse.ok) {
    ...
    throw new Error(`Falha no login do CITSMART: ${details || loginResponse.statusText}`)
  }

  const loginBodyText = await loginResponse.text()
  const cookies = mergeCookies(
    new Map<string, string>(preflightCookies),
    extractCookies(getSetCookieHeaders(loginResponse), ["JSESSIONID", "AUTH-TOKEN"]),
  )
  ...
  if (!cookieHeader) {
    throw new Error("Cookies de autenticação não encontrados no login do CITSMART.")
  }
...
export async function citSmartRequest<T>(params: {
  ...
  if (!response.ok) {
    ...
    throw new Error(`CITSMART ${response.status}: ${details || response.statusText}`)
  }
